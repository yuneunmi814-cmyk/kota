...
- **완료(백엔드)**: 스키마·마이그레이션(`20260613150000_marketplace`), 작성/검수 API(`creator/`), 마켓·이용권·구매·정산(`marketplace/`), 페이월(상세·여행 시작 게이트), 결제 어댑터(mock·PortOne v2 검증·웹훅·환불), **관리자 정산 대시보드**(`/admin/marketplace/settlements` + admin-web `/settlements`), **통합 테스트 25개**(총 84개 통과).
- **완료(모바일)**: 마켓 탐색·상세 페이월(청약철회 동의 체크)·코스 작성기·내 코스·구매함, 통합 검색, 영문 토글, 카카오맵 WebView 번호 마커/경로선.
- **완료(출시 준비)**: Docker 풀스택·CI·EAS·법무 초안·[출시 인허가 가이드](legal/출시_인허가_가이드.md). 사업자 등록 완료(프로젝트윤·**간이과세자 → 통신판매업 신고 면제**).
- **잔여**: ① **위치기반서비스사업 신고**(방통위, 출시 선행조건 — 사업자 본인) ② **PG 실연동 마무리**(PortOne 키 + 모바일 RN SDK 결제창; 유료 보류 중) ③ **구글 인앱결제 정책**(디지털콘텐츠 판매 시, 유료화 전 결정) ④ 카카오맵 WebView 실기기 렌더 검증 ⑤ 스토어 심사 제출.
