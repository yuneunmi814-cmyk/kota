...
// 로컬 DB의 실축제(TOURAPI + STDFEST + MANUAL)를 prisma/seed-festivals.json 으로 베이크 — seed:prod가 프로덕션 첫 기동 때 적재
...
async function main() {
  ...
  const fs = await prisma.festival.findMany({
    where: { source: { in: ['TOURAPI', 'STDFEST', 'MANUAL'] }, endDate: { gte: today } },
