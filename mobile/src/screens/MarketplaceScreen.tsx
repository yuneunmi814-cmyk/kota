  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      {loading ? (
        ...
        <EmptyState text={'아직 공유된 여행팩이 없어요.\n첫 여행팩을 만들어 자랑해보세요!'} />
      ) : (
        <FlatList
            ...
            <Pressable onPress={() => navigation.navigate('CourseDetail', { courseId: c.id })}>
              <Card>
                ...
                <View style={{ padding: space(3), gap: 5 }}>
                  <Text style={{ fontWeight: '700', fontSize: 14, color: colors.text }}>{c.title}</Text>
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 2 }}>
                    ...
                    {c.saveCount > 0 && <Text style={{ fontSize: 12, color: colors.textHint }}>· ♡ {c.saveCount.toLocaleString()} 저장</Text>}
                  </View>
                </View>
              </Card>
            </Pressable>
