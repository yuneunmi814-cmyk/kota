...
패키지여행식 관광가이드 앱 (안드로이드 우선). 단일 진실 공급원은 [docs/KOTA_기획설계서.md](docs/KOTA_기획설계서.md) — 화면/DB/API/정책 변경 시 문서도 함께 갱신할 것.
...
- `web/` — **코타 웹(공모전 MVP)**, React + Vite + TS + Tailwind v4. 외국인(태·일) 대상 지역축제 여행팩 웹 — 다국어 UI(ko/en/th/ja, `src/i18n.tsx`), 디자인 토큰은 전도준 시안2(화이트 배경·딥그린 `--color-green` #004027·핀 레드 `--color-pin`, Pretendard) → `src/index.css` @theme. 홈·목록 헤드라인 = **"내 여행지 주변 축제"**(8/9 회의 — 외국인은 방한 전 목적지 기준 검색, '내 위치'는 해외에서 무의미). 헤더에 **내 위치/여행지 입력**(시군구 축제 중심좌표 기반, `destinationCentroids`) 상시 배치. 상세 조회는 **externalId 기반**(`/festivals/external/:externalId`) — 정적 목록의 숫자 id는 환경(로컬/Render)마다 달라 카드↔상세 불일치 사고가 났었음(8/9). 목록은 번호 페이지네이션(?page= URL — 뒤로가기 유지). 홈 = 타이틀 + 통합 검색(목록 페이지에도 동일 검색창) + **권역 7개 필터**(`regionGroups.ts` 1단 + 하위 시·도 2단, 축제 수 표시). 목록은 **정적 베이크 전체(724건)를 클라이언트에서 필터·정렬·더보기(12개씩)** — API 24건 제한으로 12건만 보이고 거리순이 전국에 안 먹히던 8/6 QA BUG-03/04의 해법이자 콜드 스타트 무관 즉답. 법적 고지 3종(개인정보·약관·**면책조항**)은 4개 언어 완전 번역(`LegalPage.tsx` DOCS 맵). 포스터 없는 축제는 랜덤 사진 대신 브랜드 플레이스홀더 — **축제 단일 축**(호텔·맛집 탭 없음, 결정 2026-07-22). 라우트 `/`(홈) · `/festivals`(목록) · `/festivals/:id`(**상세** — 기간·장소·문의·길찾기·주변 관광지 3km) · `/search` · `/privacy`·`/terms`(법적 고지, 공통 푸터에서 링크). 시·도 탭·지역 라벨은 `sidoI18n.ts` 정적 맵으로 번역(17개 고정이라 안전). **목적 테마**(`themes.ts` 백엔드 분류 + `/themes/:key` 랜딩 6종 — 실제 여행은 '목적→어디로' 순서라는 8/12 팀 인터뷰 반영. 지역 축과 별개 탐색축이며 "가족과 갈 만한 축제"류 검색어(SEO)와 AI 질문(GEO)의 착지점. 프리렌더·사이트맵·llms.txt 포함) + **찜**(`wishlist.ts` — localStorage, 키는 환경 불변 externalId) + 목록 `?wish=1` 필터 + 상세 **캘린더 추가**(.ics·구글캘린더, 서버 불필요) — 8/12 팀 결정 '찜한 축제 알람'의 웹 1단계이며 푸시 알림은 앱 단계 과제. `/api` 프록시 :4000 (`cd web && npm run dev`, :5174). **라이브: GitHub Pages https://yuneunmi814-cmyk.github.io/kota/ → Render API `https://kota-api-109d.onrender.com`(VITE_API_BASE 주입)**. 배포 시 `web/scripts/prerender.mjs`가 **축제 상세 724건을 실파일로 프리렌더**(제목·OG·JSON-LD·noscript 본문) — 네이버 Yeti가 JS를 안 돌려 CSR을 빈 문서로 보는 문제와 SPA 폴백의 HTTP 404 문제를 함께 해결(여기울산 yw8837/ulsan_store 실측 참고). noscript에는 화면에 실제 보이는 정보만(클로킹 금지). 주변 관광지는 **적응형 반경**(3km→4곳 미만이면 10km). API 폴백 3단: 라이브 API(6초 타임아웃) → 베이크 정적 데이터 → 하드코딩. Render 무료라 콜드 스타트 시 정적 폴백
...
npm test             # vitest 통합 테스트 (kota_test DB에 migrate deploy 후 실행)
...
npm run sync:kfes [-- --dry-run]              # 구석구석 축제 큐레이션 → DB. contentid 병합 + 이름 중복 스킵, 신규만 detailCommon2로 이미지·주소
npm run classify:themes [-- --dry-run]        # 여행 목적 테마 분류(festivals.themes) — 문체부 유형 + 이름 키워드, 멱등
...
- 축제: `festivals` 테이블(전국 커버, 기간 본질이라 Spot과 분리, 좌표·regionId nullable). **5개 소스** — TourAPI(searchFestival2 + areaBasedList2, `sync.ts`) + **구석구석 kfes**(`sync:kfes` — 같은 공사 CMS인데 searchFestival2가 놓치는 큐레이션 축제, cmsCntntsId==contentid라 `tourapi:` externalId로 정확 병합, 요청 시 locationx="null" 문자열 필수·UA/Referer 필요) + 전국문화축제표준데이터(`stdfest.ts`) + **문체부 연간 개최계획**(`sync:mcst`, source=MCST — 전체 1,266건 중 예정·확정일자만, 연 1회 엑셀이라 2월경 새 연도관 수동 갱신; 좌표 없음·이름 기준 타소스 중복 스킵) + 수기(MANUAL). `externalId`(`tourapi:`/`stdfest:`) 멱등 upsert, 소스 간 **정규화 이름+시작일**로 중복 제거(`regionMap.ts` — 연도 접두 `2026 ○○축제`도 같은 축제로 판정). **시·도 정규화 필수**: 소스마다 표기가 달라(`전남광주통합특별시`·축약형 `강원`) `parseSidoSigungu`가 정식 17개 명칭으로 통일 — 통합 표기는 구(區)면 광주, 시·군이면 전남으로 분해. 화면 필터는 **`sido` 기준**(`GET /festivals/sidos`로 목록·개수 제공, `?sido=충청남도`) — 웹이 지역을 하드코딩하면 백엔드에 없는 slug로 0건이 뜨므로 **데이터에서 목록을 만든다**. `regionId`(큐레이션 23개)는 하위호환용 `?region=`에만. 통합 검색 `GET /search`에 축제 포함(축제명·시·도·시군구·주소 부분일치, 진행중+예정만). 베이크(`bake:festivals`, 예정만)를 seed-prod가 매 기동 upsert. **주간 자동 동기화**: `.github/workflows/sync-festivals.yml`(월 09:00 KST) → 동기화·재분류·베이크·커밋 → Pages·Render 자동 배포. 유지보수: `npm run fix:festival-regions`(파싱 규칙 변경 시 기존 행 교정+중복 정리)
- **축제 다국어(en/ja/th)**: 타겟이 일본·동남아인데 UI만 번역하면 축제명을 못 읽는다. 관광공사 영문 API(EngService2)는 축제 **0건** 제공(2026-08-05 실측)이라 직접 번역해 `festival_translations`에 적재. 원문은 `prisma/festival-translations.json`(축제명으로 매칭 — id는 재동기화로 바뀌지만 이름은 유지). API는 `?lang=en|ja|th`, 응답에 `name`(번역)·`nameKo`(원문)·`lang`(실제 적용 언어)·`placeName`(외국인용 지명). **번역 없으면 한국어로 폴백**해 부분 번역 상태에서도 목록이 깨지지 않는다. 베이크·정적 내보내기·seed-prod 모두 번역을 싣는다 — **콜드 스타트 동안 보이는 정적 폴백에서도 번역이 나와야** 외국인에게 의미가 있다
- **수기 축제(source=MANUAL)**: 공공 API에 아예 없는 지역 행사를 담는 통로 — 현수막·보도자료로만 알려지는 재단/시청 주관 행사(2026-08-05 실측: '의림지 달빛 물맞이 행차'가 TourAPI `searchKeyword2` '의림지'·'물맞이' 모두 0건). `prisma/manual-festivals.json` 편집 → `npm run sync:manual` → `bake:festivals`. **seed-prod의 prune 대상은 TOURAPI·STDFEST뿐이라 수기 데이터는 재배포에도 지워지지 않는다** — 이 분리가 수기 등록의 전제
