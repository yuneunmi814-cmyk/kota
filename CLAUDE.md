...
- 축제: `festivals` 테이블(기간이 본질이라 Spot과 분리, 좌표 nullable). 공개 API `GET /festivals`(기본 진행중+예정, 복합 커서 `YYYY-MM-DD_id`)·`/festivals/calendar`(날짜별 개수)·`/festivals/:id`(PostGIS 반경 3km 주변 스팟). 동기화 `src/modules/festivals/`, 시드에 데모 축제 3건(상대 날짜)
