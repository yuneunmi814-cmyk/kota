...
> ℹ️ 아래 본문은 현재 구현된 **KOTA 앱(코스 추천·가이드 모드)** 기준입니다. 공모전 방향은 위 회의를 거쳐 **코타(외국인·지역축제 웹앱)**로 진화 중이며, 두 흐름은 공공데이터 큐레이션 엔진을 공유합니다.
...
| **주제** | 코타(Korea Festa) — 외국인(태국·일본)에게 한국 **지역축제**를 공공데이터로 큐레이션하는 여행팩 웹앱 |
| **제품 범위** | 웹(공모전 제출물)은 **축제 단일 축** — 호텔·맛집·교통 탭 없음 (결정 2026-07-22). 교통·식당은 발전 계획으로 서술 |
...
- **26.07.01.** — 데일리 미팅 #4: 코타 웹에 식당·지도·검색·로그인/리뷰·추천 알고리즘·약관·모바일 반응형까지 붙음. **1인 개발/배포(윤은미)+디버깅 리포트(최용우)+검수·디자인(전도준)** 체계 확정. 디자인은 *구석구석을 반면교사로 단순·한국적*, **소셜보다 웹 최소 최적화 우선**. 로그인은 **카카오+구글+라인**(※ 실제 구현은 **카카오·구글 2종**, 라인 미구현 — 2026-08-04 확인), 번아웃 방지 **1일 1시간 리밋**. "현재 수준+버그만 잡아도 공모전 충족, 그 이상은 사업". → [회의록 전문](docs/공모전기획/팀회의_회의록_2026-07-01.md)
- **26.07.29.** — 오프사이트(용훈 별장): 공모전 웹앱을 넘어 **팀 운영 자체를 AI로 자동화하는 조직**으로 확장 논의. **마케팅 엔진(바이럴) 자동화**가 핵심 축, 상장·엑시트 대비 **COO(히스토리·회계 투명성)**의 중요성 강조. 팀을 **CTO(윤은미)·COO(최용우)·CMO(전도준)**로 재편하고, 각자 **본인 포지션 관점의 MD 파일**을 만들어 다음 회의 때 폴더로 통합하기로. COO가 **자동화 워크플로우 시제품(n8n/API)**, 데이터 축적은 **구글 시트/옵시디언/디스코드** 후보로 검토. → [회의록 전문](docs/공모전기획/팀회의_회의록_2026-07-29.md)
- **26.08.03.** — C-level MD 파일 중간 점검: 진도 정체(7월 커밋 -77%)를 확인하고 운영 체계 재정비. **미팅 주 3회**, **회의록 정리는 COO 이관**(CTO는 개발 집중), **git push는 CTO 전담**(COO·CMO는 pull만), 할 일·데일리 리포트는 **구글 시트** / MD·회의록 전문은 **GitHub**로 배치 확정. COO 범위에서 **인사·경영지원·CEO 전략 제외**(3인 스타트업 전제). 타겟은 **일본·동남아 유지**. CTO 실사용에서 **지역축제 데이터 공백** 발견 → 축제 소스 2종 통합(121 → 416건). **검색 탭 추가·지역 시·도 단위 정리**가 CTO 액션. → [회의록 전문](docs/공모전기획/팀회의_회의록_2026-08-03.md)

**C-level 문서 세트** · [docs/C-level/](docs/C-level/) — 07-29 결정에 따라 각 포지션이 작성한 전략 문서를 2026-08-04에 통합.
[CMO 전도준](docs/C-level/00_CMO_전도준/) (시장·타겟·포지셔닝 10편) · [COO 최용우](docs/C-level/01_COO_최용우/) (운영·자동화·QA 4편) · [CTO 윤은미](docs/C-level/02_CTO_윤은미/) (아키텍처·데이터·로드맵·ADR 9편). 압축본은 [_archive/](docs/C-level/_archive/).
...
```bash
git clone https://github.com/yuneunmi814-cmyk/kota.git
cd kota
```
...
- **Homebrew PostgreSQL + PostGIS** (로컬 설치형)
  ```bash
  createdb kota_dev && createdb kota_test
  # .env: DATABASE_URL=postgresql://<사용자>@localhost:5432/kota_dev
  ```
- **Docker** (PostGIS·Redis 컨테이너)
  ```bash
  docker compose up -d       # PostGIS :5433, Redis :6379
  # .env: DATABASE_URL=postgresql://kota:kota@localhost:5433/kota_dev
  ```
...
npm test                # vitest 통합 테스트 97개 (kota_test DB)
...
| 이메일 | 역할 | 비밀번호 |
|---|---|---|
| `super@kota.app` | 총괄 관리자 | `kota-dev-1234` |
| `editor@kota.app` | 콘텐츠 매니저(작성) | `kota-dev-1234` |
| `reviewer@kota.app` | 콘텐츠 매니저(검수·발행) | `kota-dev-1234` |
...
- [x] 백엔드 API(인증 RTR·탐색·저장·여행/체크인·리뷰·관리자 4-eyes·감사로그) — 테스트 97개
...
- [x] **부가 공공데이터 연동** — 관광사진(스팟 갤러리)·영문 i18n(스팟 상세 ?lang=en)·지역 방문자수(인기 정렬/배지). 제주 라이브 검증
- [x] **M3 마감** (2026-06-13) — 관심테마(ON-02)·구글 로그인·FCM 토큰 등록 완료. 카카오맵 커스텀 마커는 라이브러리 미지원(내장 POI 대체, WebView+JS SDK는 후속)
- [x] **커뮤니티 여행팩 공유(설계서 7장)** — 사용자가 코스를 만들어 **무료 공유·자랑**. 작성·검수(4-eyes)·둘러보기(인기/최신)·저장, 홈 커뮤니티 섹션. **v1 무료 전용** — 유료 결제·이용권·페이월·정산 코드는 보존된 채 비활성(유료화 시 스위치). 백엔드 테스트 97개(누적). 출시 절차: [docs/legal/출시_인허가_가이드.md](docs/legal/출시_인허가_가이드.md)
...
- [x] **클라우드 배포** — 백엔드 Render 라이브(`kota-api-109d.onrender.com`)·웹 GitHub Pages(`yuneunmi814-cmyk.github.io/kota/`)·법무 약관 Netlify 호스팅·Android EAS 빌드(.aab)
